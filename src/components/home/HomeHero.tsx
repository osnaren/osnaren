...
export function HomeHero() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-line" aria-label="Introduction">
      ...
      <div className="relative mx-auto grid min-h-[calc(100svh-140px)] max-w-[108rem] items-center gap-8 px-5 py-10 sm:px-8 lg:grid-cols-[minmax(400px,0.82fr)_minmax(0,1.28fr)] lg:gap-8 lg:py-10 xl:grid-cols-[minmax(460px,0.78fr)_minmax(0,1.34fr)] xl:gap-12 xl:px-14 2xl:px-20">
        {/* ── identity column ─────────────────────────────────────────── */}
        <div className="flex min-w-0 flex-col">
          ...
          <motion.h1
            {...enter(0.16)}
            className="mt-5 max-w-3xl text-5xl leading-[1.02] font-semibold tracking-normal text-balance sm:text-[58px] lg:text-[52px] xl:text-[62px] 2xl:text-[72px]"
          >
            I build useful frontend products from everyday problems.
          </motion.h1>

          <motion.p {...enter(0.24)} className="text-muted mt-5 max-w-xl text-[16px] leading-7 sm:text-[17px]">
          ...
          {/* active-module context — updates on hover / keyboard focus, no layout shift */}
          <motion.div
            {...enter(0.32)}
            data-testid="module-inspector"
            className="border-line-strong bg-surface/80 relative mt-6 min-h-22 overflow-hidden rounded-md border px-4 py-3 shadow-[0_3px_0_var(--line)] backdrop-blur-sm"
            aria-live="polite"
            aria-atomic="true"
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-faint font-mono text-[9px] tracking-[0.08em] uppercase">{activeModule.status}</span>
            </div>
          ...
          <motion.div {...enter(0.5)} className="mt-6 flex flex-wrap gap-3">
            <Link
              href="/projects"
